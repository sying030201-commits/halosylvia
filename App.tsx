            ...
            <div className="flex flex-col items-center mb-12 relative">
              ...
              <div className="relative flex items-center bg-bg-deep/80 backdrop-blur-xl p-2 rounded-[24px] border border-white/20 shadow-[0_0_40px_rgba(0,0,0,0.3)] group/toggle">
                ...
                <motion.div 
                  ...
                  className="absolute inset-y-2 rounded-[18px] transition-all duration-700 ease-[cubic-bezier(0.23,1,0.32,1)] left-2 w-[calc(50%-8px)] bg-then shadow-[0_0_25px_rgba(74,85,104,0.4)]"
                />
                <button 
                  onClick={() => setMode('THEN')}
                  className="relative z-10 px-10 py-4 rounded-[18px] text-[11px] font-black tracking-widest transition-all duration-500 flex items-center gap-3 text-white scale-105"
                >
                  <Clock size={16} className="animate-pulse" /> 过去：手动时代
                </button>
                <button 
                  onClick={() => setMode('NOW')}
                  className="relative z-10 px-10 py-4 rounded-[18px] text-[11px] font-black tracking-widest transition-all duration-500 flex items-center gap-3 text-text-muted hover:text-white"
                >
                  <Zap size={16} /> 现在：AI 进化
                </button>
              </div>
          ...
          >
            ...
            <div className="flex flex-col items-center mb-12 relative">
              ...
              <div className="relative flex items-center bg-bg-deep/80 backdrop-blur-xl p-2 rounded-[24px] border border-white/20 shadow-[0_0_40px_rgba(0,0,0,0.3)] group/toggle">
                ...
                <motion.div 
                  layoutId={`toggle-bg-${work.id}`}
                  className="absolute inset-y-2 rounded-[18px] transition-all duration-700 ease-[cubic-bezier(0.23,1,0.32,1)] left-[calc(50%+2px)] w-[calc(50%-8px)] bg-now shadow-[0_0_25px_rgba(196,167,125,0.4)]"
                />
                <button 
                  onClick={() => setMode('THEN')}
                  className="relative z-10 px-10 py-4 rounded-[18px] text-[11px] font-black tracking-widest transition-all duration-500 flex items-center gap-3 text-text-muted hover:text-white"
                >
                  <Clock size={16} /> 过去：手动时代
                </button>
                <button 
                  onClick={() => setMode('NOW')}
                  className="relative z-10 px-10 py-4 rounded-[18px] text-[11px] font-black tracking-widest transition-all duration-500 flex items-center gap-3 text-bg-deep scale-105"
                >
                  <Zap size={16} className="animate-pulse" /> 现在：AI 进化
                </button>
              </div>
